...
class App extends React.Component {
  ...
  render() {
    return ( 
      <div className="min-h-screen flex justify-center items-center add-background">
        <div id='quote-box' className='flex-col flex items-center justify-center rounded-xl p-8 bg-gray-300 bg-clip-padding backdrop-filter backdrop-blur-md bg-opacity-30 border border-gray-200'>
          <p id='text' className='text-white max-w-md text-lg'>{quotes[this.state.quoteIndex]['quote']}</p>
          <p id='author' className='
            my-2 text-white
          '>{quotes[this.state.quoteIndex]['author']}</p>
          <div className='flex row place-content-between w-full'>
            <button id='new-quote' onClick={this.newQuote} className='
              bg-opacity-0 hover:bg-white hover:text-black text-white font-bold py-2 px-4 rounded-lg border border-gray-200
            '>New Quote</button>
            <a id='tweet-quote' href={`https://twitter.com/intent/tweet?hashtags=quotes&related=freecodecamp&text=${quotes[this.state.quoteIndex]['quote']}-${quotes[this.state.quoteIndex]['author']}`} className='
              bg-opacity-0 hover:bg-white hover:text-black text-white font-bold py-2 px-4 rounded-lg border border-gray-200
            '>Tweet</a>
          </div>
        </div>
